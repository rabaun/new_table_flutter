              ...
              itemBuilder: (context, index) {
                return Column(
                  children: [
                    ...
                    _buildListItem('Название организации',
                    ...
                    _buildListItem(
                        'Название параметра', intensityWorkProcess?[index].parameterName ?? ''),
                    _buildListItem('Плотность сигналов (световых, звуковых) и сообщений',
                        intensityWorkProcess?[index].signalDensity ?? ''),
                    _buildListItem('Число производственных объектов, наблюдаемых одновременно',
                        intensityWorkProcess?[index].simultaneousObjectsCount ?? ''),
                    _buildListItem('Процент времени использования оптических приборов',
                        intensityWorkProcess?[index].opticalDeviceUsage ?? ''),
                    _buildListItem('Нагрузка на голосовой аппарат (суммарное количество часов в неделю)',
                        intensityWorkProcess?[index].voiceApparatusLoadHours ?? ''),
                    _buildListItem('Нагрузка на слуховой анализатор',
                        intensityWorkProcess?[index].auditoryAnalyzerLoad ?? ''),
                    _buildListItem('Длительность сосредоточенного наблюдения (в % от времени смены)',
                    ...
                    _buildListItem('Число элементов (приемов), необходимых для выполнения задания',
                        intensityWorkProcess?[index].elementsCount ?? ''),
                    _buildListItem('Уровень монотонности производственной обстановки',
                        intensityWorkProcess?[index].monotonyEnvironment ?? ''),
                    _buildListItem('Время активного наблюдения за ходом производственного процесса ',
